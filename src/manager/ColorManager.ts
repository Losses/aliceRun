...
export const ColorManager = () => {
    const initialThemeNumber = Object.values(themeHct[THEME_ID.value]).flatMap((x: Hct) => [x.hue, x.chroma, x.tone]);
    const initialTheme = {} as ITheme<Hct>;

    Object.entries(themeHct[THEME_ID.value]).forEach(([key, hct]: [string, Hct]) => {
        initialTheme[key as keyof ITheme<number>] = Hct.from(hct.hue, hct.chroma, hct.tone);
    });

    const [lerpTheme] = useLerps(initialThemeNumber, (x) => {
        Object.entries(initialTheme).forEach(([key, hct]: [string, Hct], index) => {
            hct.hue = x[index * 3];
            hct.chroma = x[index * 3 + 1];
            hct.tone = x[index * 3 + 2];
            THEME_VALUE.value[key as keyof ITheme<number>] = hct.toInt() & ARGB_TO_RGB_MASK;
        });
    ...
    THEME_ID.subscribe((id) => {
        ...
        lerpTheme(Object.values(targetTheme).flatMap((x: Hct) => [x.hue, x.chroma, x.tone]));
    });
}