...
export const HpManager = (effects: Effects) => {
    ...
    HP.subscribe((x) => {
        if (x === trueHp || trueHp > HP.value) {
            $hpVal.setAttribute('style', `width: ${HP.value}%; background: linear-gradient(90deg, rgba(255, 255, 255, 0.0), rgba(255, 255, 255, 0.7))`);
        } else {
            $hpVal.setAttribute('style', `width: ${HP.value}%; background: linear-gradient(90deg, rgba(255, 0, 0, 0.0), rgba(255, 0, 0, 0.8))`);
        }
    });
    ...
    const tick = (time: number) => {
        ...
        effects.sepiaPass.uniforms['amount'].value = Math.min(1, 1 * effectFactor * 2);
        ((effects.filmPass.uniforms as { nIntensity: any })['nIntensity']).value = 0.8 * effectFactor;
        ...
        if (time - lastCalculateTime > 400) {
            const bleeding = SPM.value < TRUE_LOW_LIMIT.value || SPM.value > TRUE_HIGH_LIMIT.value;
