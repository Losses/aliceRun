import { Madgwick } from '../../../utils/Madgwick';

const leftMadgwick = Madgwick(10);
const rightMadgwick = Madgwick(10);
const rad2deg = 180.0 / Math.PI;

type Iteratee = (value: number) => number | undefined;

function baseSum(array: number[], iteratee: Iteratee) {
    let result = 0;
...
function baseMean(array: number[], iteratee: Iteratee) {
...
const ControllerType: Record<number, string> = {
    0x1: 'Left Joy-Con',
    0x2: 'Right Joy-Con',
    0x3: 'Pro Controller',
};
...
const lastValues: Record<number, ParseResult> = {
    8198: {
        timestamp: -1,
        alpha: 0,
        beta: 0,
...
export function toEulerAngles(gyroscope: IPoint3, accelerometer: IPoint3, productId: number) {
    const now = Date.now();
...
export function toEulerAnglesQuaternion(q: IPoint4) {
    const ww = q.w * q.w;
    const xx = q.x * q.x;
    const yy = q.y * q.y;
    const zz = q.z * q.z;
...
export function toQuaternion(gyro: IPoint3, accl: IPoint3, productId: number) {
    if (productId === 0x2006) {
 ...
 */
function toAcceleration(value: IBuffer) {
 ...
 */
function toDegreesPerSecond(value: IBuffer) {
    const view = new DataView(value.buffer);
 ...
 */
function toRevolutionsPerSecond(value: IBuffer) {
    const view = new DataView(value.buffer);
    return parseFloat((0.0001694 * view.getInt16(0, true)).toFixed(6));
}

export function parseDeviceInfo(rawData: Uint8Array, data: string[]) {
...
export function parseInputReportID(rawData: Uint8Array, data: string[]) {
    const inputReportID = {
...
export function parseTimer(rawData: Uint8Array, data: string[]) {
    const timer = {
